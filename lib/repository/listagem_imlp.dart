...
class ListagemImpl {
  final HasuraConnect connect = HasuraConnect("http://192.168.1.2:8080/v1/graphql");
  ...
  Future isCheck(ListModel model) async {
    connect.mutation(updateCheckQuery,variables: {
      "id": model.id,
      "check": !model.check
    });
  }
  Future adicionar() async {
    connect.mutation(adicionarListagemQuery,variables: {
      "titulo": controller.titulo.text,
      "subtitulo": controller.subtitulo.text
    });
    controller.titulo.clear();
    controller.subtitulo.clear();
  }
  Future deletar(ListModel model) async {
    connect.mutation(deletarListagemQuery,variables: {
      "id": model.id
    });
  }
}
